...
export function Home({ user, go, cep, onEditCep, onLogout, onLoadExample }: { user: User; go: (s: string) => void; cep: CepData | null; onEditCep: () => void; onLogout: () => void; onLoadExample: () => void }) {
  ...
  const openAppearanceMenu = () => {
    const label = (c: string) => (c === "light" ? "Claro" : c === "dark" ? "Escuro" : "Automático (sistema)");
    Alert.alert(
      "Aparência",
      `Atual: ${label(choice)}`,
      [
        { text: "Claro", onPress: () => setChoice("light") },
        { text: "Escuro", onPress: () => setChoice("dark") },
        { text: "Automático (sistema)", onPress: () => setChoice("system") },
  ...
  const openAccountMenu = () => {
    Alert.alert("Sua conta", user.email || "", [
      { text: "Cancelar", style: "cancel" },
      { text: "Aparência", onPress: openAppearanceMenu },
  ...
  return (
    <Screen testID="home-screen">
      ...
      <View style={styles.tiles}>
        <Action styles={styles} colors={colors} testID="home-tile-projects" icon="folder-outline" title="Meus projetos" text="Continuar editando" onPress={() => go("projects")} />
      ...
      <View style={[styles.tiles, { marginTop: 12 }]}>
        <Action styles={styles} colors={colors} testID="home-tile-cart" icon="basket-outline" title="Carrinho" text="Compare lojas" onPress={() => go("cart")} />
        <Action styles={styles} colors={colors} testID="home-tile-alerts" icon="notifications-outline" title="Alertas" text="Fique de olho no preço" onPress={() => go("alerts")} />
      </View>
...
function Action({ icon, title, text, onPress, testID, styles, colors }: any) {
  return (
...
function Step({ n, title, text, styles }: any) {
  return (
